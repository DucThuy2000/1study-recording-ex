...
async function rehydrate(): Promise<void> {
  try {
    ...
    view =
      session.status === 'RECORDING' ? { kind: 'RECORDING', sessionId: session.sessionId } : { kind: 'STARTING' };
  } catch (error) {
...
stopBtn.addEventListener('click', () => {
  void (async () => {
    ...
    await browser.runtime.sendMessage({ type: 'STOP_RECORDING', sessionId } satisfies Message);
  })();
});
